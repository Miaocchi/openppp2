...
`geo-rules` 是一个可选配置块，默认关闭。开启后，客户端启动时会先按需下载并解析 `geoip.dat` / `geosite.dat`，再读取文本格式的 GeoIP / GeoSite 输入，生成两个文件：
...
因此，Phase G 不替换原有 `client.dns-rules`。对于 bypass，若未启用 `geo-rules`，原有 `client.bypass` / `--bypass` 行为保持不变；若启用 `geo-rules`，命令行或配置传入的 bypass 文件会被读入生成器，与 GeoIP/dat/text/append-bypass 结果合并去重后写入一个最终 `output-bypass` 文件，再只注册这个合并后的文件。
...
注意：当前 dat 文件会下载到本地缓存路径，并按 `geo-rules.country` 自动解析 `geoip.dat` / `geosite.dat` 生成规则；文本 `geoip` / `geosite` 来源、命令行/配置 bypass、以及追加项会继续叠加并去重。
...
generated bypass = geo-rules.geoip CIDR
                 + geo-rules.geoip-dat country CIDR
                 + client.bypass / --bypass file content (when geo-rules.enabled=true)
...
generated dns-rules = geo-rules.geosite -> /<domestic-provider>/nic
                    + geo-rules.geosite-dat country domains -> /<domestic-provider>/nic
...
1. 若 `geo-rules.enabled=false`，按旧逻辑直接注册原有 `client.bypass` / `--bypass`。
2. 始终注册原有 `client.routes`。
3. 先加载原有 `client.dns-rules` / `--dns-rules`。
4. 若 `geo-rules.enabled=true`，生成 Phase G 文件：`client.bypass` / `--bypass` 会进入 `output-bypass` 合并去重，而不是单独重复注册。
5. 将生成的 bypass 文件注册到现有 IP list 加载路径。
6. 将生成的 DNS rules 文件追加加载到现有 DNS rule 表。
